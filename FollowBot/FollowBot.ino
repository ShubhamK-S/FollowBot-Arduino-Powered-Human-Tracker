...
if((distance > 3) && (distance < 10)){            //check wheather the ultrasonic sensor's value stays between 1 to 15.
  ...
  //Turn Left                                                
  Motor1.setSpeed(190);  //define motor1 speed:
  Motor1.run(FORWARD);   //rotate motor1 cloclwise:
  Motor2.setSpeed(190);  //define motor2 speed:
  Motor2.run(FORWARD);   //rotate motor2 clockwise:
  Motor3.setSpeed(190);  //define motor3 speed:
  Motor3.run(BACKWARD);  //rotate motor3 anticlockwise:
  Motor4.setSpeed(190);  //define motor4 speed:
...
}else if(distance > 10) {                          //If the condition is 'true' then the statement below will execute:
